...
require '../usuarios/comprobarSesion.php';
...
function enviarImg(){
    ...
    elseif (isset($_FILES['imgpro'])) {
        foreach ($_FILES['imgpro']['name'] as $key ){
            print_r($_FILES['imgpro']['name'][$key]);
       
            if ($_FILES['imgpro']['name'][$key]) {
                
                $image = $_FILES['imgpro']['tmp_name'][$key]; 
                    $nombre_inagen=$_FILES['imgpro']['name'][$key];
                    $tipo_inagen = $_FILES['imgpro']['type'][$key];
                    echo $tipo_inagen;
                   //ruta de la carpeta destino del server
                    ...
                    $insertar = $dbd->prepare("INSERT into imagen (imagenfoto, producto_idproducto) VALUES ('$nombre_inagen','10')");
                    $insertar->execute();
                    // COndicional para verificar la subida del fichero
                    if($insertar){
