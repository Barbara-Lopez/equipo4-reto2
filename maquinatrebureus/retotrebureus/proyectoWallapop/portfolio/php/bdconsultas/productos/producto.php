...
function selectLikeProductos($patron){
    $db= connect();
    //busar todas las categorias disponibles
    $stmt = $db->query("SELECT * FROM producto WHERE titulo LIKE '%$patron%'"); //peligro inyeccion sql
    $db= close(); 
    return json_encode($stmt->fetchAll(PDO::FETCH_ASSOC));
}
...
function buscaridmaxprod(){
    ...
    $result = $db->query("SELECT max(idproducto) as idproducto FROM producto");
    $result->execute();
    $Datos = $result->fetch();
